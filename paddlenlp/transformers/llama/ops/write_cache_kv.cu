...
template <paddle::DataType D>
std::vector<paddle::Tensor> LaunchWriteCacheKV(const paddle::Tensor& input_kv, 
    ...
    const int64_t bsz = input_kv.shape()[1]; 
    const int64_t cache_bsz = input_kv.shape()[1]; 
    const int64_t num_head = input_kv.shape()[2]; 
    ...
    k_ptr = reinterpret_cast<const DataType_*>(input_kv.data<data_t>()), 
    v_ptr = k_ptr + k_size;
...
std::vector<paddle::DataType> WriteCacheKVInferDtype(const paddle::DataType& input_kv_dtype, 
                                                 const paddle::DataType& cache_kv_dtype,
                                                 const paddle::DataType& sequence_lengths_dtype) {
    return {cache_kv_dtype};
}

PD_BUILD_OP(write_cache_kv)
    .Inputs({"input_kv", "cache_kv", "sequence_lengths"})
    .Outputs({"cache_kv_out"})
    .SetKernelFn(PD_KERNEL(WriteCacheKV))
    .SetInferShapeFn(PD_INFER_SHAPE(WriteCacheKVInferShape))
    .SetInferDtypeFn(PD_INFER_DTYPE(WriteCacheKVInferDtype));